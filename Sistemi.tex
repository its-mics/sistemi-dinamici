...
% Aggiungi questo nel preambolo, dopo la definizione dei tuoi colori
\usepackage{hyperref}
\usepackage{wrapfig}
\usepackage{rotating}
\usepackage{multirow}
...
% --- CONFIGURAZIONE LAYOUT CLASSICO (Sezione a sinistra, Pagina a destra) ---
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{} % Reset completo
\usepackage{amssymb} % Required for \mathbb
\newcommand{\R}{\mathbb{R}}
% Intestazione
\renewcommand{\headrulewidth}{0.4pt} % Linea sottile
\fancyhead[L]{\small\textit{\leftmark}} % Sezione a sinistra
\fancyhead[R]{\small\thepage}           % Numero pagina a destra

% Piè di pagina
\fancyfoot{} % Completamente vuoto per massima pulizia
% --------------------------------------------------------------------------
...
\begin{itemize}
    ...
    \item \textbf{Sistemi a Tempo Discreto (Mappe):} Descritti da relazioni di ricorrenza $x_{n+1} = f(x_n)$, dove il tempo avanza a scatti discreti ($n=1, 2, 3...$).
\end{itemize}
...
\begin{enumerate}
    \item $f(x^*; r^*) = 0$ (Equilibrio)
    \item $\frac{\partial f}{\partial x}(x^*; r^*) = 0$ (Non iperbolicità)
    \item $\frac{\partial^2 f}{ \partial r}(x^*; r^*) \neq 0$ 
\end{enumerate}
...
\subsection*{Parte facoltativa: Varietà Differenziabili (Facoltativo)}
...
\[ V = \sum_{i=1}^m V^i \frac{\partial}{\partial x^i}\bigg|_P \quad \text{dove} \quad V^i = \frac{dx^i}{dt}(0) \]
...
\begin{tikzpicture}[scale=0.8]
    ...
    \foreach \c in {0.3, 0.8, 1.5}{
        ...
        % Quadranti superiori
        \draw[green!50!black, thick, ->-=.5] plot[domain=2.8:0.1, samples=40] ({\c*(\x/2.8)^2}, \x);
        \draw[green!50!black, thick, ->-=.5] plot[domain=2.8:0.1, samples=40] ({-\c*(\x/2.8)^2}, \x);
        % Quadranti inferiori
        \draw[green!50!black, thick, ->-=.5] plot[domain=2.8:0.1, samples=40] ({\c*(\x/2.8)^2}, -\x);
        \draw[green!50!black, thick, ->-=.5] plot[domain=2.8:0.1, samples=40] ({-\c*(\x/2.8)^2}, -\x);
...
\begin{itemize}
    ...
    \item Verso $\lambda = 4$ si entra in una regione con pattern non riconoscibile \textbf{(Caos)}.
\end{itemize}
...
$$ T^2(x) = \begin{cases} 
2(2x) & 0 \leq 2x \leq \frac{1}{2} \\
2(1-2x) & \frac{1}{2} \leq 2x \leq 1 \\
2(1-x) & 0 \leq 2(1-x) \leq \frac{1}{2} \\
2 - 2(2-2x) & \frac{1}{2} \leq 2(1-x) \leq 1 
\end{cases}
\quad = \quad
\begin{cases}
...
Siano $f: I \to I$ e $g: J \to J$ coniugate tramite $h: I \to J$. \\
...
Fissiamo un tempo arbitrario $s \in \R$. Sfruttando la continuità del flusso dinamico, possiamo scrivere:
...
\begin{equation}
    \dot{V}(\vb{x}) = \eval{\dv{t} V(\phi_t(\vb{x}))}_{t=0} = \pdv{V}{x_1}\dv{x_1}{t} + \dots + \pdv{V}{x_n}\dv{x_n}{t} = \grad V \cdot \vb{f}(\vb{x})
\end{equation}
...
\begin{enumerate}
    ...
    \item Guardiamo il bordo di questa palla, $\partial B_\varepsilon(\vb{x}^*)$. Poiché $V$ è definita positiva e continua, su questo bordo (che è un compatto) $V$ assumerà un valore minimo. Chiamiamo questo minimo $m$:
    $$ m = \min_{\vb{x} \in \partial B_\varepsilon} V(\vb{x}) $$
    Poiché $V(\vb{x})$ è strettamente maggiore di $V(\vb{x}^*)$ fuori dal punto fisso, sappiamo sicuramente che $m > V(\vb{x}^*)$.
    
    \item Definiamo un "insieme sicuro" $S_m$ all'interno della palla:
    $$ S_m = \{ \vb{x} \in B_\varepsilon(\vb{x}^*) \mid V(\vb{x}) < m \} $$
    Poiché $V(\vb{x}^*) < m$, il punto fisso è contenuto dentro $S_m$. Per continuità della funzione $V$, esisterà sicuramente un raggio $\delta > 0$ abbastanza piccolo tale che la palla $B_\delta(\vb{x}^*)$ sia \textbf{tutta} contenuta in $S_m$.
...
\begin{tikzpicture}[
    scale=1.5,
    % Stili personalizzati per le frecce sulle curve blu
    arr_fwd/.style={postaction={decorate, decoration={markings,
        mark=at position 0.4 with {\arrow[scale=1.2]{Stealth}},
        mark=at position 0.75 with {\arrow[scale=1.2]{Stealth}}}}},
    arr_bwd/.style={postaction={decorate, decoration={markings,
        mark=at position 0.25 with {\arrow[scale=1.2]{Stealth}},
        mark=at position 0.6 with {\arrow[scale=1.2]{Stealth}}}}},
    arr_fwd_single/.style={postaction={decorate, decoration={markings,
        mark=at position 0.6 with {\arrow[scale=1.2]{Stealth}}}}},
    arr_bwd_single/.style={postaction={decorate, decoration={markings,
        mark=at position 0.4 with {\arrow[scale=1.2]{Stealth}}}}}
]
    ...
    % Assi X e Y
    \draw[thick] (-3.9,0) -- (3.9,0) node[below] {$x$};
    \draw[thick] (0,-3.9) -- (0,3.9) node[left] {$y$};

    % Frecce Verdi sull'Asse X (Autospazio Stabile E^s, puntano verso l'origine)
    \path[postaction={decorate, decoration={markings,
        mark=at position 0.15 with {\arrow[green!60!black, scale=1.4]{Stealth}},
        mark=at position 0.35 with {\arrow[green!60!black, scale=1.4]{Stealth}},
        mark=at position 0.65 with {\arrow[green!60!black, scale=1.4]{Stealth}},
        mark=at position 0.85 with {\arrow[green!60!black, scale=1.4]{Stealth}}
    }}] (-4,0) -- (0,0);
    \path[postaction={decorate, decoration={markings,
        mark=at position 0.15 with {\arrow[green!60!black, scale=1.4]{Stealth}},
        mark=at position 0.35 with {\arrow[green!60!black, scale=1.4]{Stealth}},
    ...
    % Frecce Verdi sull'Asse Y (Autospazio Instabile E^u, puntano lontano dall'origine)
    \path[postaction={decorate, decoration={markings,
        mark=at position 0.15 with {\arrow[green!60!black, scale=1.4]{Stealth}},
        mark=at position 0.35 with {\arrow[green!60!black, scale=1.4]{Stealth}},
        mark=at position 0.65 with {\arrow[green!60!black, scale=1.4]{Stealth}},
    ...
    % Disegnata in due metà per gestire correttamente il verso delle frecce
    \draw[thick, red, domain=0:3.8, samples=100, postaction={decorate, decoration={markings,
        mark=at position 0.25 with {\arrow[scale=1.3]{Stealth}},
        mark=at position 0.6 with {\arrow[scale=1.3]{Stealth}},
        mark=at position 0.9 with {\arrow[scale=1.3]{Stealth}}
    }}] plot (\x, {-\x*\x/3});
    
    \draw[thick, red, domain=-3.8:0, samples=100, postaction={decorate, decoration={markings,
        mark=at position 0.1 with {\arrow[scale=1.3]{Stealth}},
        mark=at position 0.4 with {\arrow[scale=1.3]{Stealth}},
        mark=at position 0.75 with {\arrow[scale=1.3]{Stealth}}
    }}] plot (\x, {-\x*\x/3});

    % Testo dell'equazione della parabola
    \node[red, font=\large] at (-2, -3.4) {$y = -\frac{x^2}{3}$};
    ...
    % Traiettoria interna 1
    \draw[thick, blue, domain=0.2:3.8, samples=100, arr_bwd] plot (\x, {0.8/\x - \x*\x/3});
    \draw[thick, blue, domain=-3.8:-0.2, samples=100, arr_fwd] plot (\x, {0.8/abs(\x) - \x*\x/3});

    % Traiettoria media 2
    \draw[thick, blue, domain=0.6:4, samples=100, arr_bwd] plot (\x, {2.5/\x - \x*\x/3});
    \draw[thick, blue, domain=-4:-0.6, samples=100, arr_fwd] plot (\x, {2.5/abs(\x) - \x*\x/3});

    % Traiettoria esterna 3
    \draw[thick, blue, domain=1.1:4, samples=100, arr_bwd_single] plot (\x, {5/\x - \x*\x/3});
    \draw[thick, blue, domain=-4:-1.1, samples=100, arr_fwd_single] plot (\x, {5/abs(\x) - \x*\x/3});


    % --- Curve INFERIORI alla parabola (K < 0) ---
    % Traiettoria interna 1 (U rovesciata stretta)
    \draw[thick, blue, domain=0.12:3.5, samples=100, arr_bwd] plot (\x, {-0.5/\x - \x*\x/3});
    \draw[thick, blue, domain=-3.5:-0.12, samples=100, arr_fwd] plot (\x, {-0.5/abs(\x) - \x*\x/3});

    % Traiettoria esterna 2 (U rovesciata larga)
    \draw[thick, blue, domain=0.4:3.8, samples=100, arr_bwd] plot (\x, {-2/\x - \x*\x/3});
    \draw[thick, blue, domain=-3.8:-0.4, samples=100, arr_fwd] plot (\x, {-2/abs(\x) - \x*\x/3});
...
\]
è il grafico di una funzione tangenziale ad $E^s$ nell'origine. Pertanto $W^{(s)}_{loc}$ è una varietà differenziabile e il suo spazio tangente nell'origine coincide con il sottospazio lineare $E^s$:
\[
T_0 W^{(s)}_{loc} = E^s
\]
